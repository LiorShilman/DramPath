...
### תרגול חופשי לפי תווים (מסך חדש, `/practice/visual/free-notation`)

**רעיון המשתמש:** מוד נוסף בתרגול הוויזואלי — מעלים תמונה/PDF של תווים לשיר כלשהו, מקישים על המקשים הממופים תוך כדי קריאת התווים, שומעים את הכלי המתאים ורואים את ערכת התופים מגיבה, עם מטרונום אופציונלי לקצב. בניגוד לתרגילים המדורגים (`InteractiveExercise`/`DrumNoteEvent[]`), לתמונה/PDF שהועלו אין נתוני-תווים מובנים — **אין ניקוד אוטומטי** (לא Accuracy/Combo/Perfect-Early-Late-Miss), זהו כלי תרגול חופשי בלבד, סוכם מראש עם המשתמש.

- **שימוש חוזר כמעט מלא ברכיבים קיימים:** `useKeyboardDrums` (שלב 3), `playDrumSound` (שלב 2), `DrumKit`, ו-`useMetronome` (הקיים מ-`practice-session`, כולל `AudioContext` נפרד משלו — לא שיתוף עם ה-context של הקשות התופים, כדי לא לסכן את ה-hook הקיים בשימוש אחר). הקוד החדש היחיד המשמעותי: `src/hooks/useFreeDrumPlayback.ts` — hook קטן שיוצר `AudioContext`+`GainNode` בעצלנות (בהקשה ראשונה, לפי כלל ה-gesture), קורא ל-`playDrumSound` בזמן אמת, וחושף `activeHit` להנפשת הפגיעה.
- **תמיכה ב-PDF, לא רק תמונות:** התגלה שכל קבצי התווים האמיתיים של המשתמש הם PDF (לא תמונות) — נוסף רינדור PDF דרך `<iframe src={url}#view=FitH>` (משתמש בצפייה המובנית של הדפדפן, בלי תלות ב-pdf.js). `#view=FitH` נדרש כי ברירת המחדל ("fit page") השאירה שוליים שחורים גדולים ולפעמים גלשה מעבר לפריים בעמוד גבוה מרחב.
- **איטרציית Layout ארוכה (RTL):** מספר סבבים על "צד ימין/שמאל" ו"עולה על התפריט" — הפתרון הסופי: `lg:flex-row-reverse` (לא `lg:flex-row`) כדי שהילד הראשון ב-DOM (עמודת התווים) יישב משמאל, בעוד עמודת ערכת-תופים→מטרונום→מקשים (מוטמעת, לא ה-`KeyboardGuide` הקבוע-בתחתית המשותף) יושבת מימין צמוד לתפריט הצד. ערכת התופים נשמרת ב-`w-[80%] mx-auto` בתוך העמודה שלה (לא `w-full`) כי חלקי המצילות מתוכננים לגלוש מעט מעבר לקונטיינר שלהם במכוון — ברוחב מלא הגלישה הזו נוחתת ישירות על התפריט.
- **תקציב גובה viewport:** נמדד בפועל (לא ניחוש) עם Playwright במספר גבהי viewport — התצוגה (`h-[75vh]`) + `pb-12` בתחתית העמוד מתאימים ל-90%+ מגדלי חלון סבירים בלי גלילה אנכית.

**בדיקות:** `useFreeDrumPlayback.test.ts` (3, עם אותו `FakeAudioContext` מ-`useVisualTrainer.test.ts`) — הקשה ממופה מעדכנת `activeHit` עם `hitToken` חדש בכל הקשה, מקש לא ממופה לא עושה כלום.
