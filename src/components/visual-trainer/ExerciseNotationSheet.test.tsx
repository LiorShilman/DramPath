...
describe('ExerciseNotationSheet', () => {
  ...
  it('positions kick with a filled circle notehead, low on the staff', () => {
    ...
    expect(Number(circle.getAttribute('cy'))).toBe(66) // baseline(72) - position(1)*6
  })
  ...
  it('renders crash and hihat as X noteheads (two crossed lines, no circle)', () => {
    ...
    expect(crash.querySelectorAll('line')).toHaveLength(3) // ledger + 2 crossed lines
    expect(hihat.querySelector('circle')).toBeNull()
    expect(hihat.querySelectorAll('line')).toHaveLength(2) // 2 crossed lines, no ledger

    const crashY = Number(crash.querySelector('line')!.getAttribute('y1'))
    ...
    expect(crashY).toBeLessThan(hihatY) // crash sits higher (lower cy) than hihat
  })
